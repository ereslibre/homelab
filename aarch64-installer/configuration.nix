# SD/USB installer image for headless aarch64 Pi bring-up.
#
# Baked-in ereslibre SSH key on root so a fresh Pi powered on with this
# image plugged in is immediately reachable from hulk over its known
# static-DHCP IP (no HDMI / keyboard required).
#
# Build:   just installer
# Decompress & flash to a USB-SSD, plug into the rack Pi, power on,
# ssh root@<dhcp-ip>.
{modulesPath, ...}: {
  imports = [
    (modulesPath + "/installer/sd-card/sd-image-aarch64-installer.nix")
  ];

  users.users.root.openssh.authorizedKeys.keys = [
    "ssh-rsa  ereslibre@kde.org"
  ];
}
