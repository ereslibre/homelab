function fish_prompt --description 'Write out the prompt'
	...
	case '*_vi_*' '*_vi'
        ...
        if begin; [ $USER = "ereslibre" ]; and [ $__fish_prompt_hostname = "Rafaels-MacBook-Pro" ]; end
          ...
          if [ $USER = "ereslibre" ]
            set __fish_prompt_user_and_hostname (set_color $fish_color_host) "$__fish_prompt_hostname" $normal ' '
          else
            set __fish_prompt_user_and_hostname (set_color $fish_color_user) "$USER" $normal @ (set_color $fish_color_host) "$__fish_prompt_hostname" $normal ' '
          end
        end

	echo -n -s $__fish_prompt_user_and_hostname (set_color $color_cwd) (prompt_pwd) $normal (__fish_git_prompt) $normal $prompt_status "$mode_str" " > "
end
